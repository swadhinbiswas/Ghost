...
var homeIgnorePatterns = sync.OnceValue(func() []gitignore.Pattern {
	...
	for _, name := range []string{
		...
		filepath.Join(homeDir, ".config", "ghost", "ignore"),
	} {
...
type directoryLister struct {
	// dirPatterns caches parsed patterns from .gitignore/.ghostignore for each directory.
...
// .gitignore and .ghostignore files. Results are cached.
func (dl *directoryLister) getDirPatterns(dir string) []gitignore.Pattern {
	return dl.dirPatterns.GetOrSet(dir, func() []gitignore.Pattern {
		...
		for _, ignoreFile := range []string{".gitignore", ".ghostignore"} {
