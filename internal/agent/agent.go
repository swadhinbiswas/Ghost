...
import (
	...
	"github.com/swadhinbiswas/ghost/internal/stringext"
...
var userAgent = fmt.Sprintf("Charm-Ghost/%s (https://charm.land/Ghost)", version.Version)
...
func (a *sessionAgent) Run(ctx context.Context, call SessionAgentCall) (*fantasy.AgentResult, error) {
	...
	result, err := agent.Stream(genCtx, fantasy.AgentStreamCall{
		...
		StopWhen: []fantasy.StopCondition{
			func(_ []fantasy.StepResult) bool {
				cw := largeModel.CatwalkCfg.ContextWindow
	...
	if err != nil {
		...
		// INFO: we use the parent context here because the genCtx has been canceled.
...
func (a *sessionAgent) getCacheControlOptions() fantasy.ProviderOptions {
	if t, _ := strconv.ParseBool(os.Getenv("GHOST_DISABLE_ANTHROPIC_CACHE")); t {
...
If not, please feel free to ignore. Again do not mention this message to the user.`,
	...
	for _, m := range msgs {
		...
		// Assistant message without content or tool calls (canceled before it
		// returned anything).
