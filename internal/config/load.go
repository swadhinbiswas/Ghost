...
func PushPopGhostEnv() func() {
	var found []string
	for _, ev := range os.Environ() {
		if strings.HasPrefix(ev, "GHOST_") {
			pair := strings.SplitN(ev, "=", 2)
			if len(pair) != 2 {
				continue
			}
			found = append(found, strings.TrimPrefix(pair[0], "GHOST_"))
		}
	}
	...
	for _, ev := range found {
		os.Setenv(ev, os.Getenv("GHOST_"+ev))
	}
...
func (c *Config) setDefaults(workingDir, dataDir string) {
	...
	if str, ok := os.LookupEnv("GHOST_DISABLE_PROVIDER_AUTO_UPDATE"); ok {
...
func GlobalConfig() string {
	if GhostGlobal := os.Getenv("GHOST_GLOBAL_CONFIG"); GhostGlobal != "" {
		return filepath.Join(GhostGlobal, fmt.Sprintf("%s.json", appName))
	}
...
func GlobalConfigData() string {
	if GhostData := os.Getenv("GHOST_GLOBAL_DATA"); GhostData != "" {
		return filepath.Join(GhostData, fmt.Sprintf("%s.json", appName))
	}
...
func GlobalSkillsDirs() []string {
	if GhostSkills := os.Getenv("GHOST_SKILLS_DIR"); GhostSkills != "" {
		return []string{GhostSkills}
	}
	...
	// On Windows, also load from app data on top of `$HOME/.config/Ghost`.
...
// ProjectSkillsDir returns the default project directories for which Ghost
// will look for skills.
func ProjectSkillsDir(workingDir string) []string {
	return []string{
		filepath.Join(workingDir, ".agents/skills"),
		filepath.Join(workingDir, ".ghost/skills"),
